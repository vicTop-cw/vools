...
from vools.reactive._compat import curry, lazy
...
class PipeBuilder(Generic[T]):
    ...
    def to_blocking(self) -> Any:
        ...
        if error:
            raise error[0]
        if not result:
            raise ValueError("Observable did not emit any value")
        return result[0]
...
class Observable(Generic[T]):
    ...
    def to_list(self) -> "PipeBuilder[T]":
        """返回链式管道构建器并绑定 to_list 操作符"""
        from ..operators import to_list
        return self.pipe.to_list()
