...
try:
    ...
    pytest.skip(f"bridge_manager.json 权限错误，跳过测试: {e}", allow_module_level=True)
except RuntimeError as e:
    if "restricted" in str(e) or "sandbox" in str(e).lower():
        pytest.skip(f"沙箱限制，跳过测试: {e}", allow_module_level=True)
    pytest.skip(f"环境问题导致无法运行验证测试: {e}", allow_module_level=True)
