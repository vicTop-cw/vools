...
try:
    ...
    pytest.skip(f"无法导入 vools.bridge.java: {e}", allow_module_level=True)
...
def test_java_compiler_available():
    """测试编译器可用性检查"""
    result = java_compiler_available()
    assert isinstance(result, bool)
    print(f"Java 编译器 (javac) 可用: {result}")
    if not result:
        pytest.skip("Java 编译器不可用，跳过后续编译执行测试")


def test_java_runtime_available():
    """测试 Java 运行时可用性"""
    assert is_java_available() is True or is_java_available() is False
    print(f"Java 运行时 (java) 可用: {is_java_available()}")


def test_type_mapping():
    """测试类型映射"""
    # Python → Java 类型
    assert PY_TO_JAVA_TYPE[int] == 'int'
    assert PY_TO_JAVA_TYPE[float] == 'double'
    assert PY_TO_JAVA_TYPE[str] == 'String'
    assert PY_TO_JAVA_TYPE[bool] == 'boolean'
    assert PY_TO_JAVA_TYPE[bytes] == 'byte[]'
...
def test_async_mode_only_code():
    ...
    @java(mode='ONLY_CODE', async_mode=True)
...
if __name__ == '__main__':
    ...
    available = java_compiler_available()
    print(f"Java 编译器可用: {available}")
