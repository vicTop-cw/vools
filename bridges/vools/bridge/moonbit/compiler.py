...
'''
    ...
    def compile_code(self, code: str, func_name: str, cache_dir: Optional[str] = None) -> str:
        ...
        self._store_metadata(code, project_dir, func_name)

        return project_dir

    def _store_metadata(self, code: str, project_dir: str, func_name: str):
        """从生成的代码中提取函数元数据并存储"""
        try:
            parts = code.split('\nfn main')
            if len(parts) >= 2:
                func_def_code = parts[0].strip()
                params = []
                ret_type = 'String'

                for line in func_def_code.split('\n'):
                    line = line.strip()
                    if line.startswith('fn ') and '(' in line and ')' in line:
                        sig = line[line.index('(')+1:line.index(')')]
                        if sig.strip():
                        ...
                        if '->' in line:
                            ret_part = line.split('->')[-1].strip().rstrip('{').strip()
                            ret_type = ret_part
                        break

                self._func_metadata[project_dir] = {
    ...
    def call_func(self, lib_path: str, func_name: str,
        ...
        从存储的元数据中获取函数信息，将实际参数注入 main.mbt 后运行。
        """
        metadata = self._func_metadata.get(lib_path)
        if metadata:
            self._write_main_with_args(
                lib_path,
                metadata['func_def_code'],
                metadata['func_name'],
                metadata['params'],
                metadata['ret_type'],
                args,
