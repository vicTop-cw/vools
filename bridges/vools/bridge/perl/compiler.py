...
# 解析 Perl 命令：优先本地解释器，否则尝试 WSL
_PERL_CMD, _PATH_CONVERTER, _USE_WSL = resolve_command(_PERL_COMPILER)


def perl_compiler_available() -> bool:
    """检查 Perl 解释器是否可用（执行 `perl -v`）"""
    try:
...
def _execute_perl_code(code: str, func_name: str, cache_dir: str = None,
    ...
    converted_src_path = _PATH_CONVERTER(src_path)

    if not force and os.path.exists(src_path):
        result = subprocess.run(
            _PERL_CMD + [converted_src_path],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
'''
            ...
            wrapper_file = os.path.join(tmpdir, f'run_{func_name}.pl')
            with open(wrapper_file, 'w', encoding='utf-8') as f:
                f.write(wrapper_code)

            cmd = _PERL_CMD + [_PATH_CONVERTER(wrapper_file)]

            result = subprocess.run(cmd, capture_output=True, text=True,
                                    errors='replace', timeout=30)
            if result.returncode != 0:
                raise RuntimeError("Execution failed: " + result.stderr)
            return _parse_perl_output(result.stdout.strip(), perl_ret_type)
        finally:
    ...
    def _generate_function(self, spec: FunctionSpec) -> str:
        ...
        params_str = ', '.join(f'${name}' for name in arg_names) if arg_names else ''
    ...
    def run_project(self, project_dir: str, entry: str = 'main',
        ...
        if entry == 'main':
            ...
            result = subprocess.run(
                _PERL_CMD + [_PATH_CONVERTER(main_pl)] + list(args),
                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                text=True, errors='replace',
                timeout=60,
            )
