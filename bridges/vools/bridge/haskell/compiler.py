...
# GHC 链接器全局锁，避免并发链接时争用输出文件
_ghc_lock = threading.Lock()
...
def _compile_haskell_code(code: str, func_name: str, cache_dir: str = None,
    ...
    # 加锁编译，并对链接器权限/并发问题做重试
    max_retries = 3
    last_error = None
    with _ghc_lock:
        for attempt in range(max_retries):
            # 确保源文件在调用 GHC 前存在（可能被并发清理）
            ...
            if 'permission denied' in err_lower or 'failed to write' in err_lower:
    ...
    raise RuntimeError(
        f'Haskell 编译失败:\n'
        f'stderr:\n{last_error[0] if last_error else ""}\n'
        f'stdout:\n{last_error[1] if last_error else ""}\n'
        f'代码:\n{final_code}'
    )
