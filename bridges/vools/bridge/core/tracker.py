...
class CompileTracker:
    ...
    def _connect(self):
        """建立数据库连接并设置内存 journal 模式。

        部分运行环境（如 TRAE 沙箱）会限制对数据库 journal 文件
        （bridge_records.db-journal）的写入，导致 SQLite 报
        "unable to open database file"。使用 MEMORY journal 模式
        可避免创建磁盘 journal 文件，同时保留基本的事务语义。
        """
        conn = sqlite3.connect(self._db_path)
        try:
            conn.execute('PRAGMA journal_mode=MEMORY;')
        except Exception:
            # 如果设置失败，继续以默认模式运行，由外层逻辑报告错误
            pass
        return conn
