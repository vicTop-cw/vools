...
PowerShell language bridge for vools.

## Status

- **Decorator:** `@powershell`, `@ps`
...
- PowerShell (`powershell`) on Windows, or `pwsh` on Linux/macOS
- PowerShell installed and available in PATH

## Notes

- PowerShell is an interpreted language; code is executed via subprocess
- Uses JSON serialization for data exchange between Python and PowerShell
- `@ps` is an alias for `@powershell`
- Built-in on Windows; requires PowerShell Core (`pwsh`) on Linux/macOS