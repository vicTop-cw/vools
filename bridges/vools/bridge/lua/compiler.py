...
# Lua 解释器候选
_LUA_COMPILER_CANDIDATES = ('lua', 'lua5.3', 'luajit')


def _resolve_lua_command() -> tuple:
    """解析可用的 Lua 解释器，返回 (cmd_prefix, path_converter, use_wsl)"""
    for name in _LUA_COMPILER_CANDIDATES:
        cmd_prefix, path_converter, use_wsl = resolve_command(name)
        try:
            ...
            continue
    # 无可用解释器时，回退到第一个候选（保持与 resolve_command 行为一致）
    return resolve_command(_LUA_COMPILER_CANDIDATES[0])


_LUA_CMD, _PATH_CONVERTER, _USE_WSL = _resolve_lua_command()

# 保护 _execute_lua_code 中同名临时文件的并发写入
_lua_exec_lock = threading.Lock()
...
def _lua_long_bracket(value: str) -> str:
    """生成 Lua 长括号字符串，避免内容与结束标记冲突"""
    level = 0
    while True:
        close = ']' + '=' * level + ']'
        prefix = ']' + '=' * level
        if close not in value and not value.endswith(prefix):
            open_ = '[' + '=' * level + '['
            return f'{open_}{value}{close}'
        level += 1
...
def _generate_lua_source(
    ...
    # Lua 5.1/5.3 兼容
    code_parts.append('local unpack = unpack or table.unpack')
    code_parts.append('')

    # JSON 解析（Lua 5.3+ 内置）
    code_parts.append('-- 解析参数')
    code_parts.append(f'local args_json = {_lua_long_bracket(args_json)}')
    code_parts.append('local ok, json = pcall(require, "json")')
    code_parts.append('if not ok then json = {} end')
...
def _execute_lua_code(code: str, func_name: str, cache_dir: str = None,
    ...
    with _lua_exec_lock:
        if not force and os.path.exists(src_path):
            result = subprocess.run(
                _LUA_CMD + [_PATH_CONVERTER(src_path)],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
class LuaBridge(LangBridge):
    ...
    def compile_project(self, project_dir: str, entry: str, output_dir: str = None) -> str:
        ...
        if entry == 'main':
            ...
            all_code.append('')
            all_code.append('local unpack = unpack or table.unpack')
            all_code.append('')
    ...
    def run_project(self, project_dir: str, entry: str = 'main',
        ...
        if entry == 'main':
            ...
            result = subprocess.run(
                _LUA_CMD + [_PATH_CONVERTER(main_lua)] + list(args),
                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                text=True, errors='replace',
                timeout=60,
            )
