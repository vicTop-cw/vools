...
# 查找系统上可用的完整 MinGW GCC（供 Nim 后端使用）
def _find_mingw_gcc() -> str:
    """返回完整 MinGW GCC 可执行文件路径，找不到则返回 'gcc'"""
    ...
    for mp in mingw_paths:
        gcc_path = os.path.join(mp, 'gcc.exe')
...
def _compile_nim_code(code: str, func_name: str, cache_dir: str = None) -> str:
    ...
    # 将 nimcache 指向缓存目录，避免默认写入用户主目录且权限不足
    nimcache_dir = os.path.join(cache_dir, 'nimcache')
    os.makedirs(nimcache_dir, exist_ok=True)

    # 编译命令
    nim_path = _get_nim_path()

    if _IS_WINDOWS:
        # 使用 GCC 后端；Nim 的 {.exportc.} 配合 -d:release 与 --app:lib
        # 已能正确导出符号，无需手动 /EXPORT 链接器标志。
        compile_cmd = [
            nim_path, 'c', '--app:lib',
            '--out:' + dll_path,
            '--nimcache:' + nimcache_dir,
            '-d:release',
            '--cc:gcc',  # 优先使用 GCC 后端，避免 VCC 头文件缺失
            '--passL:-Wl,--export-all-symbols',  # GCC 下导出所有符号
            nim_file
        ]
    else:
        compile_cmd = [
    ...
    # 执行编译（Windows 上确保使用完整 MinGW GCC，避免 bundled gcc 缺少头文件）
    import subprocess
    env = os.environ.copy()
    if _IS_WINDOWS:
        gcc_path = _find_mingw_gcc()
        gcc_dir = os.path.dirname(gcc_path)
        if gcc_dir and os.path.isfile(gcc_path):
...
def _generate_nim_wrapper(func_name: str, args: tuple, nim_body: str,
    ...
    # 生成导出函数 - 使用 exportc + dynlib 确保 Windows DLL 正确导出符号
    if actual_ret_type == 'void':
        code = f'''proc {func_name}*({params_str}) {{.exportc: "{func_name}", dynlib.}} =
{indented_body}'''
    else:
        code = f'''proc {func_name}*({params_str}): {actual_ret_type} {{.exportc: "{func_name}", dynlib.}} =
{indented_body}'''
    return code
...
class NimBridge(LangBridge):
    ...
    def _generate_function(self, spec: FunctionSpec) -> str:
        ...
        if ret_type == 'void':
            return f'''proc {spec.name}*({params_str}) {{.exportc: "{spec.name}", dynlib.}} =
{indented_body}'''
        else:
            return f'''proc {spec.name}*({params_str}): {ret_type} {{.exportc: "{spec.name}", dynlib.}} =
{indented_body}'''
