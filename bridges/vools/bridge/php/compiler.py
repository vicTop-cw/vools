...
# ----------------------------------------------------------------------------
# PHP 解释器解析（本地优先，WSL 兜底）
# ----------------------------------------------------------------------------
_PHP_CMD, _PATH_CONVERTER, _USE_WSL = resolve_command('php')
...
def _execute_php_code(code: str, func_name: str, cache_dir: str = None,
    ...
    if not force and os.path.exists(src_path):
        result = subprocess.run(
            _PHP_CMD + [_PATH_CONVERTER(src_path)],
...
'''
            ...
            wrapper_file = os.path.join(tmpdir, f'run_{func_name}.php')
            with open(wrapper_file, 'w', encoding='utf-8') as f:
                f.write(wrapper_code)
            
            cmd = _PHP_CMD + [_PATH_CONVERTER(wrapper_file)]
...
'''
        ...
        try:
            result = subprocess.run(
                _PHP_CMD + [_PATH_CONVERTER(temp_script)],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                text=True, errors='replace',
                timeout=60,
            )
    ...
    def run_project(self, project_dir: str, entry: str = 'main',
        ...
        if entry == 'main':
            ...
            cmd = _PHP_CMD + [_PATH_CONVERTER(main_php)] + list(str(a) for a in args)
