...
from .core.compat.dataclass_compat import dataclass, field, asdict
...
LANGUAGE_PROBES: Dict[str, Dict[str, Any]] = {
    ...
    'kotlin': {
        ...
        'version_pattern': r'(\d+\.\d+(?:\.\d+)?)',
        'category': 'JVM',
        'wsl_shell_env': 'source ~/.sdkman/bin/sdkman-init.sh 2>/dev/null',
        'install_hint': {
...
def probe_wsl(
    ...
    if dist is None:
        ...
        dist = distros[0] if distros else 'Ubuntu'
...
def _probe_one_wsl(lang: str, config: Dict[str, Any], dist: str) -> LanguageStatus:
    ...
    shell_env = config.get('wsl_shell_env', '')

    for cmd in config['commands']:
        try:
            # 检查命令是否存在（优先直接 which，失败时尝试加载 shell 环境）
            path = None
            for use_shell in (False, True):
                if use_shell and not shell_env:
                    continue
                if use_shell:
                    check = subprocess.run(
                        ['wsl', '-d', dist, '--', 'bash', '-c', f'{shell_env}; which {cmd}'],
                        stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                        timeout=5,
                    )
                else:
            ...
            if path:
                ...
                try:
                    ...
                    if shell_env:
                        result = subprocess.run(
                            ['wsl', '-d', dist, '--', 'bash', '-c',
                             f'{shell_env}; {" ".join([cmd] + version_args)}'],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                            timeout=5,
                        )
                    else:
...
COMMON_INSTALL_PATHS: Dict[str, List[str]] = {
    ...
    'freebasic': [
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'bridge', 'freebasic', 'compiler'),
        r'C:\FreeBASIC',
...
def probe_with_extra_paths(
    ...
    report = probe_environment(languages=languages, extra_env=all_extra_env)

    # 7. Windows 上本地未探测到的语言回退到 WSL
    if _IS_WINDOWS:
        missing = [lang for lang, st in report.languages.items()
                   if not st.available and (languages is None or lang in languages)]
        if missing:
            try:
                wsl_reports = probe_all_wsl(languages=missing)
                for wsl_report in wsl_reports:
                    for lang, wsl_st in wsl_report.languages.items():
                        if wsl_st.available and lang in report.languages:
                            local_st = report.languages[lang]
                            if not local_st.available:
                                local_st.available = True
                                local_st.version = wsl_st.version or local_st.version
                                local_st.path = wsl_st.path or local_st.path
                                local_st.command = wsl_st.command or local_st.command
                                local_st.error = wsl_st.error or local_st.error
            except Exception:
                pass
