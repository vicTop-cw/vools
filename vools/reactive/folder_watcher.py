...
class _Win32WatchBackend:
    ...
    def __init__(
        self,
        on_change: Callable[[str, str | None, FolderChangeType, bool], None],
...
class FolderSubject(Subject[FolderData]):
    """一个带目录监控能力的 Subject。
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._dispatcher: FolderDispatcher = FolderDispatcher(
            paths=paths,
            backend=backend,
            change_types=change_types,
            tags=tags,
...
# ====================================================================
# FolderObserver: 按 FolderChangeType 路由的便捷观察者
# ====================================================================


class FolderObserver:
    """声明式地监听 FolderSubject / FolderDispatcher 发出的事件。

    用它代替手写 lambda + 类型判断。

    用法:
        >>> obs = FolderObserver(
        ...     on_folder_created=lambda fd: print("新增目录:", fd.path),
        ...     on_folder_deleted=lambda fd: print("删除目录:", fd.path),
        ...     on_folder_renamed=lambda fd: print(f"{fd.old_path} -> {fd.path}"),
        ...     on_any=lambda fd: print("事件:", fd.change_type.name),
        ... )
        >>> obs.subscribe(fs)
        >>> # 也可链式:
        >>> with FolderObserver(...).attach(fs):
        ...     # ... 其他代码
    """
    ...
    def __init__(
        self,
        *,
        on_folder_created: Callable[[FolderData], Any] | None = None,
        on_folder_deleted: Callable[[FolderData], Any] | None = None,
        on_folder_renamed: Callable[[FolderData], Any] | None = None,
        on_folder_moved_in: Callable[[FolderData], Any] | None = None,
        on_folder_moved_out: Callable[[FolderData], Any] | None = None,
        on_folder_attrib: Callable[[FolderData], Any] | None = None,
        on_folder_content: Callable[[FolderData], Any] | None = None,
    ...
    def _on_next(self, fd: FolderData) -> None:
        if self._on_any is not None:
            try:
                ...
                log.debug("FolderObserver.on_any 异常: %s", e)

        ct = fd.change_type
        handler = {
            FolderChangeType.FOLDER_CREATED: self._on_folder_created,
            FolderChangeType.FOLDER_DELETED: self._on_folder_deleted,
            FolderChangeType.FOLDER_RENAMED: self._on_folder_renamed,
            FolderChangeType.FOLDER_MOVED_IN: self._on_folder_moved_in,
            FolderChangeType.FOLDER_MOVED_OUT: self._on_folder_moved_out,
            FolderChangeType.FOLDER_ATTRIB: self._on_folder_attrib,
            FolderChangeType.FOLDER_CONTENT: self._on_folder_content,
        }.get(ct, None)
        ...
        if handler is not None:
            try:
                ...
                log.debug("FolderObserver 回调 %s 异常: %s", ct.name, e)
    ...
    def _on_error_handler(self, err: Exception) -> None:
        if self._on_error is not None:
            try:
                ...
                log.debug("FolderObserver.on_error 异常: %s", e)
    ...
    def _on_completed_handler(self) -> None:
        if self._on_completed is not None:
            try:
                ...
                log.debug("FolderObserver.on_completed 异常: %s", e)

    def subscribe(self, observable: Any) -> Any:
        """订阅 Observable/Subject/FolderSubject。返回 Subscription。"""
        self.unsubscribe()
    ...
    def attach(self, subject_or_dispatcher: Any) -> "FolderObserver":
