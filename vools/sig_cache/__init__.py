...
用法:
    >>> from vools.sig_cache import get_signature, add_custom_sig
    >>>
...
log = logging.getLogger("vools.sig_cache")
...
def add_custom_sig(
    ...
    *,
    ...
    用法:
        >>> from inspect import Signature, Parameter
        >>> from vools.sig_cache import add_custom_sig, get_signature
        >>>
        >>> # 为 C 扩展函数注册签名
        >>> sig = Signature([
        ...     Parameter('x', Parameter.POSITIONAL_OR_KEYWORD),
...
def remove_signature(func: Callable[..., Any]) -> bool:
    ...
    用法:
        >>> from vools.sig_cache import get_signature, remove_signature
        >>>
...
def clear_cache() -> None:
    ...
    用法:
        >>> from vools.sig_cache import clear_cache
        >>> clear_cache()
    """
    global _CACHE_HITS, _CACHE_MISSES
    _SIG_CACHE.clear()
...
def cache_info() -> dict[str, Any]:
    ...
    用法:
        >>> from vools.sig_cache import cache_info, get_signature
        >>> get_signature(len)
        >>> info = cache_info()
        >>> info["hit_ratio"]
        0.0
