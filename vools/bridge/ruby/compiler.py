...
# 常用 PATH 搜索
_RUBY_SEARCH_PATHS_WINDOWS = [
    r"C:\Ruby\bin",
    r"C:\Ruby31\bin",
    r"C:\Ruby32\bin",
    os.path.expanduser("~/Ruby/bin"),
]
_RUBY_SEARCH_PATHS_UNIX = [
    "/usr/bin",
    "/usr/local/bin",
    "/opt/homebrew/bin",
    os.path.expanduser("~/.rbenv/shims"),
    os.path.expanduser("~/.rvm/rubies/default/bin"),
]


def _setup_ruby_env() -> str:
    """设置 Ruby 运行环境（PATH）；返回 ruby 可执行路径"""
    search_paths = _RUBY_SEARCH_PATHS_WINDOWS if _IS_WINDOWS else _RUBY_SEARCH_PATHS_UNIX
    ...
    return _get_ruby_path()


def _get_ruby_path() -> str:
    """获取 ruby 解释器路径"""
    found = shutil.which(_RUBY_COMPILER)
    if found:
        return found
    ...
    for p in search_paths:
        candidate = os.path.join(p, _RUBY_COMPILER + exe_suffix)
        if os.path.exists(candidate):
            return candidate
    return _RUBY_COMPILER


_RUBY_PATH = _setup_ruby_env()
