...
从原 vicTools 类的静态方法拆分而来，提供独立的工具函数。
"""
...
def transfer(func=None, w=None):
    ...
    Returns:
        包装后的函数，其返回值会被转换为对应的类型
    """
    wraper_func = w
    if func is None:
        return lambda f, w=wraper_func: transfer(f, w=w)
    ...
    def wrapper(*args, **kwargs):
        ...
        if hasattr(result, '__class__'):
            ...
            if class_name in ('VText', 'vicText', 'VList', 'vicList'):
                return result
...
def generate_lambda(ex: str, param_symbols="x"):
    ...
    Returns:
        生成的lambda函数
    """
    param_symbols = transferCols(param_symbols)
...
def get_date_seq(nums=15, date_type='day', fmt='%m%d', run_ds=None, duo=True, reverse=True):
    ...
    if reverse:
        return get_date_seq(nums=nums, date_type=date_type, fmt=fmt, run_ds=run_ds, duo=duo, reverse=False)[::-1]

    freq = date_type[0].upper()
    run_date = datetime.strptime(run_ds, '%Y-%m-%d') if "-" in run_ds else datetime.strptime(run_ds, '%Y%m%d')

    if freq == 'D':
        dss = [d.date() for d in _generate_date_range(run_date, nums, freq='D')]
    ...
    if freq == 'W':
        ...
        dss = [d.date() for d in _generate_date_range(temp_run_date, nums, freq='W')]
    ...
    dss = [d.date() for d in _generate_date_range(end_of_month, nums, freq='M')]
...
def regexp_like(pattern, source_string, flags=0, complete_matching=True):
    ...
    Returns:
        是否匹配
    """
    matchs = regexp_findall(pattern=pattern, source_string=source_string, flags=flags)
...
def regexp_count(pattern, source_string, flags=0):
    ...
    Returns:
        匹配次数
    """
    return len(regexp_findall(pattern=pattern, source_string=source_string, flags=flags))
...
def regexp_instr(pattern, source_string, flags=0, occurrence=0):
    ...
    Returns:
        匹配的起始位置
    """
    temp = regexp_substr(pattern=pattern, source_string=source_string, flags=flags, occurrence=occurrence)
...
def regexp_split(pattern, source_string, flags=0, rep='★'):
    ...
    if source_string.count(rep) == 0 and len(rep) == 1:
        ...
        if isinstance(pattern, str):
            ...
            return _split(string=source_string, sep=list(sep), rep=rep)
...
def _split(string, sep, rep='★'):
    ...
    Returns:
        分割后的字符串列表
    """
    rep = get_char(string=string, rep=rep)
    if rep is None:
        return None
    if check_no_overlap(sep):
        for p in sep:
...
def split(string, separator, rep='★'):
    ...
    if isinstance(separator, str):
        ...
        for p in separator:
            if isinstance(p, str):
                s.add(p)
        return _split(string=string, sep=list(s), rep=rep)
    else:
        return None
