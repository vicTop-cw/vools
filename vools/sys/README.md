...
`@exe` 和 `@dll` 都支持智能的 fallback 机制，规则如下：
...
| 维度 | vools.sys (@exe/@dll) | vools.bridge (LangBridge) |
|------|----------------------|---------------------------|
| **桥接对象** | 已编译的 exe/dll 二进制文件 | 源代码字符串，自动编译 |
| **适用场景** | 调用现成的外部工具、第三方库 | 性能优化、用其他语言重写函数 |
| **编译器依赖** | 不需要编译器 | 需要对应语言的编译器 |
| **代码位置** | 外部二进制文件中 | Python 函数体中（代码字符串） |
