...
proc seq_map_int*(data: cstring; multiplier: cint): cstring {.exportc: "seq_map_int".} =
...
proc seq_filter_int*(data: cstring; threshold: cint): cstring {.exportc: "seq_filter_int".} =
  let arr = parseInts(data)
  let filtered = filterImpl[int](arr, proc(x: int): bool = x > int(threshold))
  result = filtered.mapIt($it).join(",").cstring

proc seq_reduce_sum_int*(data: cstring): cstring {.exportc: "seq_reduce_sum_int".} =
  let arr = parseInts(data)
  result = $sumImpl[int](arr)

proc seq_reduce_max_int*(data: cstring): cstring {.exportc: "seq_reduce_max_int".} =
  let arr = parseInts(data)
  if arr.len == 0:
    result = "0".cstring
  else:
...
proc seq_reduce_min_int*(data: cstring): cstring {.exportc: "seq_reduce_min_int".} =
  let arr = parseInts(data)
  if arr.len == 0:
    result = "0".cstring
  else:
...
proc seq_sort_int*(data: cstring; desc: cint): cstring {.exportc: "seq_sort_int".} =
  var arr = parseInts(data)
  sortImpl[int](arr, int(desc) != 0)
  result = arr.mapIt($it).join(",").cstring

proc seq_unique_int*(data: cstring): cstring {.exportc: "seq_unique_int".} =
  let arr = parseInts(data)
  result = uniqueImpl[int](arr).mapIt($it).join(",").cstring

proc seq_count_int*(data: cstring; threshold: cint): cstring {.exportc: "seq_count_int".} =
  let arr = parseInts(data)
  let c = countImpl[int](arr, proc(x: int): bool = x > int(threshold))
  result = $c

proc seq_reverse_int*(data: cstring): cstring {.exportc: "seq_reverse_int".} =
  let arr = parseInts(data)
  result = reverseImpl[int](arr).mapIt($it).join(",").cstring

proc seq_take_int*(data: cstring; n: cint): cstring {.exportc: "seq_take_int".} =
  let arr = parseInts(data)
  result = takeImpl[int](arr, int(n)).mapIt($it).join(",").cstring

proc seq_skip_int*(data: cstring; n: cint): cstring {.exportc: "seq_skip_int".} =
  let arr = parseInts(data)
...
proc seq_map_float*(data: cstring; multiplier: cstring): cstring {.exportc: "seq_map_float".} =
...
proc seq_filter_float*(data: cstring; threshold: cstring): cstring {.exportc: "seq_filter_float".} =
  let arr = parseFloats(data)
...
proc seq_reduce_sum_float*(data: cstring): cstring {.exportc: "seq_reduce_sum_float".} =
  let arr = parseFloats(data)
  result = $sumImpl[float](arr)

proc seq_reduce_max_float*(data: cstring): cstring {.exportc: "seq_reduce_max_float".} =
  let arr = parseFloats(data)
  if arr.len == 0:
    result = "0.0".cstring
  else:
...
proc seq_reduce_min_float*(data: cstring): cstring {.exportc: "seq_reduce_min_float".} =
  let arr = parseFloats(data)
  if arr.len == 0:
    result = "0.0".cstring
  else:
...
proc seq_sort_float*(data: cstring; desc: cint): cstring {.exportc: "seq_sort_float".} =
  var arr = parseFloats(data)
  sortImpl[float](arr, int(desc) != 0)
  result = arr.mapIt(formatFloat(it, precision = -1)).join(",").cstring

proc seq_unique_float*(data: cstring): cstring {.exportc: "seq_unique_float".} =
  let arr = parseFloats(data)
  result = uniqueImpl[float](arr).mapIt(formatFloat(it, precision = -1)).join(",").cstring

proc seq_count_float*(data: cstring; threshold: cstring): cstring {.exportc: "seq_count_float".} =
  let arr = parseFloats(data)
  let thresh = parseFloat($threshold)
  let c = countImpl[float](arr, proc(x: float): bool = x > thresh)
  result = $c

proc seq_reverse_float*(data: cstring): cstring {.exportc: "seq_reverse_float".} =
  let arr = parseFloats(data)
...
proc seq_map_string*(data: cstring; prefix: cstring; suffix: cstring): cstring {.exportc: "seq_map_string".} =
  let s = $data
...
proc seq_filter_string*(data: cstring; minLen: cint): cstring {.exportc: "seq_filter_string".} =
  let s = $data
  if s.len == 0: return "".cstring
  let items = s.split(',')
  let ml = int(minLen)
...
proc seq_sort_string*(data: cstring; desc: cint): cstring {.exportc: "seq_sort_string".} =
  var items = ($data).split(',')
  sortImpl[string](items, int(desc) != 0)
  result = items.join(",").cstring

proc seq_unique_string*(data: cstring): cstring {.exportc: "seq_unique_string".} =
  let s = $data
  if s.len == 0: return "".cstring
  let items = s.split(',')
  result = uniqueImpl[string](items).join(",").cstring

proc seq_count_string*(data: cstring; minLen: cint): cstring {.exportc: "seq_count_string".} =
  let s = $data
...
proc seq_reverse_string*(data: cstring): cstring {.exportc: "seq_reverse_string".} =
  let s = $data
  if s.len == 0: return "".cstring
  result = reverseImpl[string](s.split(',')).join(",").cstring

proc seq_take_string*(data: cstring; n: cint): cstring {.exportc: "seq_take_string".} =
  let s = $data
  if s.len == 0: return "".cstring
  result = takeImpl[string](s.split(','), int(n)).join(",").cstring

proc seq_skip_string*(data: cstring; n: cint): cstring {.exportc: "seq_skip_string".} =
  let s = $data
  if s.len == 0: return "".cstring
