...
proc isLeapYear*(year: int): cint {.exportc: "dt_is_leap_year".} =
  ## 是否闰年 - 返回 0/1（ctypes bool 兼容）
  if (year mod 4 == 0 and year mod 100 != 0) or (year mod 400 == 0): 1.cint else: 0.cint

proc daysInMonth*(year: int; month: int): int {.exportc: "dt_days_in_month".} =
  ## 指定年月的天数
  case month
...
proc daysInYear*(year: int): int {.exportc: "dt_days_in_year".} =
  ## 一年的天数
...
proc timestampToYMD*(ts: int64): cstring {.exportc: "dt_ts_to_ymd".} =
  ## 时间戳(秒) -> "YYYY-MM-DD" (UTC)
  let dt = fromUnix(ts.int).utc
  result = ($dt.year & "-" & intToStr(dt.month.ord, 2) & "-" & intToStr(dt.monthday, 2)).cstring

proc timestampToYMDHMS*(ts: int64): cstring {.exportc: "dt_ts_to_ymdhms".} =
...
proc ymdToTimestamp*(year: int; month: int; day: int): int64 {.exportc: "dt_ymd_to_ts".} =
...
proc daysBetween*(y1: int; m1: int; d1: int; y2: int; m2: int; d2: int): int {.exportc: "dt_days_between".} =
...
proc dayOfWeek*(year: int; month: int; day: int): int {.exportc: "dt_day_of_week".} =
  ## 返回星期几 (1=周一, 7=周日)
  let dt = dateTime(year, month.Month, day.MonthdayRange, 0, 0, 0, 0, utc())
  result = dt.weekday.ord + 1  # Nim: Monday=0, ... Sunday=6 -> we want 1..7

proc weekOfYear*(year: int; month: int; day: int): int {.exportc: "dt_week_of_year".} =
  ## 返回 ISO 周数（用元旦所在周推算）
  let dt = dateTime(year, month.Month, day.MonthdayRange, 0, 0, 0, 0, utc())
  # 1月4日总是在 ISO 周数的第一周
...
proc rangeDays*(startY: int; startM: int; startD: int; count: int): cstring {.exportc: "dt_range_days".} =
...
proc rangeDaysBetween*(y1: int; m1: int; d1: int; y2: int; m2: int; d2: int): cstring {.exportc: "dt_range_days_between".} =
...
proc rangeMonths*(startY: int; startM: int; count: int): cstring {.exportc: "dt_range_months".} =
...
proc validateDate*(year: int; month: int; day: int): cint {.exportc: "dt_validate_date".} =
  ## 校验日期是否合法 - 返回 0/1
...
proc addDays*(year: int; month: int; day: int; delta: int): cstring {.exportc: "dt_add_days".} =
...
proc addMonths*(year: int; month: int; day: int; delta: int): cstring {.exportc: "dt_add_months".} =
  ## 日期加 delta 月，处理溢出
  var m = month + delta
  var y = year
  while m > 12:
